...
Forty-eight tagged releases, thirty-five of them published to PyPI (the
package went out under Trusted Publishing from 1.3.1 onwards). No runtime
dependencies, Python 3.10+, 1442 tests, and a type checker that passes clean.
...
| Evaluation | `metrics`, `costs`, `independence`, `extremes` |
...
`reconcile`, `calendars`, `fx`, `ticksize`, `arrival`, `seasonality`, `resolution`, `auctions`, `independence`, `staleness`, `halts`, `coverage`, `provenance` and `extremes`. The first two were the items that stood under
